...
app.on('ready', function() {
  ...
  parent.respondTo('browser-initialize', function(opts, done) {
    ...
    /**
     * Не грузим определенные урлы (CSS например)
     */

    win.webContents.session.webRequest.onBeforeRequest(
        ['*'],
        function (details, callback) {
          callback({ cancel: details.url.match(/(css|jpeg|png|gif|pdf|jpg|eot|ttf|woff|woff2|fonts\.googleapis\.com\/|mp3|svg|vk\.com|mc\.yandex|uiscom|youtube|google\-analytics|chat\.s3\.yandex\.net)[\?a-zA-Z\dа-яА-Я\=\-\.&,\+:]*$/i) !== null })
        }
    )
